...
> 本项目 Fork 自 [F9y4ng/GreasyFork-Scripts](https://github.com/F9y4ng/GreasyFork-Scripts)，基于其中的 **「优雅的搜索引擎助手」** (`Google & Baidu Switcher.user.js`) 进行个性化定制，以适配自身使用需求。
...
| 搜索引擎 | 搜索类型 | 按钮样式 | 特色功能 |
|---------|---------|---------|---------|
| 哔哩哔哩 (Bilibili) | 综合搜索 | 蓝色渐变 (`#00a1d6` → `#00d4ff`) | 关键词高亮、广告过滤 |
| 小红书 (Xiaohongshu) | 综合搜索 | 红色渐变 (`#ff2442` → `#ff6b6b`) | 关键词高亮、广告过滤 |

新增的两个搜索引擎与原有引擎完全一致地集成，支持在所有已支持的搜索引擎之间互相跳转。
